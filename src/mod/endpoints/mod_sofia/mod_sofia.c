	...
	if (!strncasecmp(profile_name, "gateway/", 8) || !strncasecmp(profile_name, "trunk/", 6)) {
		...
		*dest++ = '\0';
		...
		if (!(gateway_ptr = sofia_reg_find_gateway(gw))) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "Invalid Trunk \'%s\'\n", gw);
			cause = SWITCH_CAUSE_INVALID_GATEWAY;
			goto error;
		}

		if (gateway_ptr->status != SOFIA_GATEWAY_UP) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR, "Trunk \'%s\' is down!\n", gw);
