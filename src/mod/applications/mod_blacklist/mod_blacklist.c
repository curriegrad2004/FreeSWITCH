...
SWITCH_STANDARD_API(blacklist_api_function)
{
	...
	if (!strcasecmp(argv[0], "check")) {
	...
	} else if (!strcasecmp(argv[0], "dump") || !strcasecmp(argv[0], "save"))  {
		switch_hash_index_t *hi;
		void *val;
		const void *var;
		blacklist_t *bl = NULL;
		...
		switch_mutex_lock(globals.lists_mutex);
		if (switch_file_open(&fd, filename, SWITCH_FOPEN_WRITE | SWITCH_FOPEN_TRUNCATE | SWITCH_FOPEN_CREATE, SWITCH_FPROT_OS_DEFAULT, globals.pool) == SWITCH_STATUS_SUCCESS) {
